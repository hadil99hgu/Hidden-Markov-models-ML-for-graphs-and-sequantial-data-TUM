...
class HMM_TxtGenerator:
    ...
    def forwards(self, x):
        ...
        ### YOUR CODE HERE ###
        alpha = np.zeros((T, K))
        alpha[0] = (pi * B[:,x[0]].reshape(K, 1) ).flatten() 
        
        # Iterate through the observation sequence and compute the forward probabilities
        for t in range(1, T):
            for k in range(K):
                alpha[t, k] = np.dot(alpha[t-1], A[:, k]) * B[k, x[t]]
    ...
    def generate_sentence(self, sentence_length):
        ...
        """
        # Initialize the list of word indices with the starting token
        A = self.model_params.A  # [KxK]
        K = A.shape[0]
        check_sequential_A = True
        check_nondeterministic_A = True

        for i in range(K):
            next_state = (i+1) % K  
            if np.count_nonzero(A[i,:] != 0) > 1:   
                check_nondeterministic_A = False
            if np.count_nonzero(A[i,:] != 0) > 2:  
                check_sequential_A = False
            elif A[i,next_state] == 0:  
                check_sequential_A = False
                
        if check_sequential_A:
            starting_state = np.argmax(self.model_params.pi)
            r = [np.random.choice(K, p=self.model_params.B[starting_state,:])]
            r = [np.random.choice(K, p=self.model_params.B[starting_state,:])]
            current_state = starting_state
            for _ in range(sentence_length - 1):
                next_state = (current_state + 1) % K
                current_state = next_state
                alpha = self.forwards([current_state])
                p = alpha[-1,:] / np.sum(alpha[-1,:])
                next_word = np.random.choice(K, p=np.ravel(p))
                r.append(next_word)
            sent = self.X_to_sentence(r)
            return sent
        
        r = [np.random.choice(K, p=np.ravel(self.model_params.pi))]


        for _ in range(sentence_length - 1):
            alpha = self.forwards(r)
            p=alpha[-1:]/np.sum(alpha[-1:])
            next_word = np.random.choice(K, p=np.ravel(p))
            r.append(next_word)
